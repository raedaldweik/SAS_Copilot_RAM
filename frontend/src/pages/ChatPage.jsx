  ...
  return (
    <div className="h-full flex gap-4 p-4">
                  ...
                  <div className="flex items-center">
                    <button onClick={() => setActiveChatId(chat.id)}
                      ...
                      style={chat.id === activeChatId
                        ? { color: 'var(--gold-lo)', background: 'rgba(185,28,44,0.14)', border: '1px solid rgba(185,28,44,0.30)', borderInlineStart: '3px solid var(--gold)' }
                        : { color: 'var(--text-md)' }
                      }>
      ...
      <div className="flex-1 glass-card flex flex-col relative" style={{ boxShadow: 'var(--glass-shadow-lg)' }}>
        <img src="/rta-logo.png" alt="" className="chat-watermark" onError={e => e.target.style.display='none'} />
            ...
            <div key={i} className={`flex gap-2.5 animate-fade-up ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              {msg.role === 'user' ? (
                ...
                <div className="w-8 h-8 rounded-lg shrink-0 flex items-center justify-center p-1"
                  ...
                  <img src="/rta-logo.png" alt="Assistant" className="w-full h-full object-contain" />
                </div>
              )}
              {/* Bubble */}
                  ...
                  <div className={`px-4 py-3 text-[13px] leading-[1.75] ${
                    ...
                    {msg.attachmentName && (
                      <div className="flex items-center gap-1.5 mb-2 px-2 py-1 rounded-md text-[11px] font-semibold w-fit"
                        style={{ background: 'rgba(185,28,44,0.10)', border: '1px solid rgba(185,28,44,0.22)', color: 'var(--gold-lo)' }}>
        ...
        {(attachment || attaching || attachError) && (
          <div className="px-5 pt-1 relative z-[1] animate-fade-up">
            {attachError ? (
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-[11.5px]"
                style={{ background: 'rgba(185,28,44,0.07)', border: '1px solid rgba(185,28,44,0.20)', color: 'var(--text-dim)' }}>
                <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: 'var(--gold)' }} />
                {t('readingDocument')}
              </div>
            ) : (
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-[11.5px] font-semibold"
                style={{ background: 'rgba(185,28,44,0.08)', border: '1px solid rgba(185,28,44,0.25)', color: 'var(--gold-lo)' }}>
        ...
        <div className="px-5 pb-4 pt-2 relative z-[1]">
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-xl border border-[rgba(15,23,42,0.10)] transition-all focus-within:border-[var(--gold-hi)] focus-within:shadow-[0_0_0_3px_rgba(185,28,44,0.10)]"
            ...
            <button onClick={() => fileRef.current?.click()} disabled={attaching}
              ...
              className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all hover:bg-[rgba(185,28,44,0.08)] disabled:opacity-40"
