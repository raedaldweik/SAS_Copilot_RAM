...
A ready-made NCGR-flavored dataset (Saudi government procurement, amounts in
SAR) generated deterministically at first use, with realistic red-flag
...
* split purchasing under the 200,000 SAR direct-award threshold at the
  Ministry of Sports,
...
SEED = 20260705
DIRECT_AWARD_THRESHOLD = 200_000  # SAR

ENTITIES = [
    "Ministry of Health", "Ministry of Education", "Ministry of Transport",
    "Ministry of Interior", "Ministry of Municipal and Rural Affairs",
    "Ministry of Energy", "Ministry of Finance", "Ministry of Sports",
    "Saudi Data & AI Authority", "Royal Commission for Riyadh City",
]
CATEGORIES = [
...
REGIONS = ["Riyadh", "Jeddah", "Dammam", "Makkah", "Madinah", "Abha", "Tabuk", "Qassim"]

# category → (typical unit price SAR, tender value log-mean)
_CAT_PROFILE = {
...
def _gen_supplier_names(rng, n):
    prefixes = ["Al-Riyadh", "Al-Noor", "Gulf", "Najd", "Arabian", "Modern",
...
def _build() -> dict[str, pd.DataFrame]:
    ...
    suppliers.loc[idx["S021"], "supplier_name"] = "Al-Riyadh Advanced Tech"
    ...
    for _, v in over_inv.sample(n=min(20, len(over_inv)), random_state=3).iterrows():
        add_alert("Price anomaly", "high", rng.uniform(70, 92), v.entity,
                  ...
                  f"Unit price {v.unit_price:,.0f} SAR is ~40% above the category "
    ...
    for _, t in split_t.iterrows():
        add_alert("Split purchasing", "medium", rng.uniform(55, 80), t.entity,
                  t.winning_supplier_id, t.tender_id,
                  f"Direct award of {t.winning_bid:,.0f} SAR sits just below the "
                  f"{DIRECT_AWARD_THRESHOLD:,.0f} SAR threshold, clustered with "
    ...
    for d in dups:
        add_alert("Duplicate invoice", "high", rng.uniform(65, 90), d["entity"],
                  ...
                  f"Invoice amount {d['amount']:,.0f} SAR duplicates an earlier "
